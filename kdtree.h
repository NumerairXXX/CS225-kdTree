...
class KDTree
{
    ...
    private:
         ...
         */
	void build(int left, int right, int dimension);
	void swap(int index1, int index2);
	int partition(int left, int right, int dimension, int pivotIndex);
	void select(int left, int right, int k, int dimension);
	Point<Dim> findhelper(const Point<Dim> & query, const Point<Dim> & target, int left, int right,int dimension) const;
};
